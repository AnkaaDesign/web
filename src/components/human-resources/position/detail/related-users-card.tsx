...
import { IconUsers, IconUser, IconMail, IconPhone, IconChevronRight, IconBuildingSkyscraper } from "@tabler/icons-react";
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">Nenhum funcionário cadastrado com este cargo</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <Badge variant="secondary">
            {position.users.length} funcionário{position.users.length !== 1 ? "s" : ""}
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="mt-4 pt-4 border-t">
          <Button variant="outline" className="w-full" onClick={() => navigate(routes.administration.users.root + `?positionId=${position.id}`)}>
            Ver todos os funcionários com este cargo
