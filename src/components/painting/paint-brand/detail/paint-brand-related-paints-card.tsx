...
interface PaintBrandRelatedPaintsCardProps {
  paintBrand: PaintBrand;
}

export function PaintBrandRelatedPaintsCard({ paintBrand }: PaintBrandRelatedPaintsCardProps) {
  const navigate = useNavigate();
  const paints = paintBrand.paints || [];
  const totalPaints = paintBrand._count?.paints || 0;
  ...
  const statistics = useMemo(() => {
    ...
    // Count unique paint types
    const uniquePaintTypes = new Set(paints.filter((p) => p.paintType?.name).map((p) => p.paintType!.name));
    const paintTypeCount = uniquePaintTypes.size;
      ...
      <Card className="shadow-sm border border-border">
          ...
          Tintas Relacionadas
            ...
            {paintBrand.id && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  const catalogUrl = routes.painting.catalog.root + "?paintBrandIds=" + paintBrand.id;
                  navigate(catalogUrl);
                }}
              >
                Ver no catálogo
        ...
        <CardContent className="pt-0">
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">Nenhuma tinta cadastrada para esta marca.</p>
    ...
    <Card className="shadow-sm border border-border">
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {statistics.paintTypeCount > 0 && (
            <Badge variant="outline" className="font-medium">
              <IconPalette className="h-3 w-3 mr-1" />
              {statistics.paintTypeCount} {statistics.paintTypeCount === 1 ? "Tipo" : "Tipos"} de Tinta
            </Badge>
          )}
