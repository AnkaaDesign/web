...
import { ColumnVisibilityManager } from "../../list/column-visibility-manager";
import { createItemColumns } from "../../list/item-table-columns";
...
export function RelatedItemsCard({ items, categoryId, className }: RelatedItemsCardProps) {
  ...
  const { visibleColumns, setVisibleColumns } = useColumnVisibility(
    "category-items-visible-columns",
    new Set(["uniCode", "name", "brand.name", "quantity", "price"])
  );
  ...
  const filteredItems = useMemo(() => {
    ...
    return safeItems.filter((item) => {
      return (
        ...
        item.brands?.some((b) => b.name.toLowerCase().includes(searchLower)) ||
      ...
      <Card className={cn("shadow-sm border border-border w-full", className)}>
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            {categoryId && (
              <Button variant="outline" size="sm" onClick={() => navigate(`${routes.inventory.products.list}?categories=${categoryId}`)}>
                Ver no estoque
              </Button>
            )}
          </div>
                    ...
                    >
                      {column.sortable !== false ? (
                        <button
                          ...
                          className={cn(
                            "flex items-center gap-1 w-full h-10 px-4 hover:bg-muted/80 transition-colors cursor-pointer text-left border-0 bg-transparent",
