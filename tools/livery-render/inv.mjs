/* Driver da sonda de INVENTÁRIO — sobe `invprobe.ts`, imprime o relatório.
   Uso: node tools/livery-render/inv.mjs
   Higiene idêntica a `map.mjs` (ver o cabeçalho de lá). */
...
const built = await esbuild.build({
  entryPoints: [join(HERE, 'invprobe.ts')],
...
const HTML = `<!doctype html><meta charset="utf-8"><title>invcheck</title>
<script type="module" src="/invprobe.js"></script>`;
...
if (err) console.log('\n!! ERRO NA PAGINA:\n' + err);
else console.log(JSON.stringify(await page.evaluate('window.__report()'), null, 1));
await browser.close();
