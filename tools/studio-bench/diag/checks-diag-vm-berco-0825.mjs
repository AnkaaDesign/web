/* SONDA — o berço do estepe do VM (`step_0_p0`) e a vizinhança dele. */
const out = [];
...
const passos = [];
cab.traverse((o) => {
  if (!/step/i.test(o.name || '')) return;
  const b = caixaZn(o);
  passos.push(`${o.name} (${o.isMesh ? 'malha' : 'nó'}, visível=${o.visible})`
    + (b.isEmpty() ? ' — sem geometria'
      : ` x ${mm(b.min.x)}…${mm(b.max.x)} y ${mm(b.min.y)}…${mm(b.max.y)} z ${mm(b.min.z)}…${mm(b.max.z)}`));
});
out.push([`1 · nós/malhas com "step" — ${passos.length}`, '\n        ' + passos.join('\n        ')]);
/* 2 · A vizinhança do estepe. */
const estepe = cab.getObjectByName('step_0_p0');
const cxE = caixaZn(estepe);
out.push(['2 · o estepe', `x ${mm(cxE.min.x)}…${mm(cxE.max.x)} y ${mm(cxE.min.y)}…${mm(cxE.max.y)}`
  + ` z ${mm(cxE.min.z)}…${mm(cxE.max.z)}`]);
const reg = cxE.clone().expandByScalar(0.25);
reg.max.y += 0.45;
const viz = [];
cab.traverse((o) => {
  if (!o.isMesh || !o.visible || !o.geometry) return;
  for (let p = o; p; p = p.parent) if (p === estepe) return;
  ...
  for (const b of caixas.values()) {
    ...
    if (Math.max(d.x, d.y, d.z) < 0.05) continue;
    viz.push({ nome: o.name, faces: b.n, box: b.box, d, cabe: reg.containsBox(b.box),
      rot: `${mm(d.x)}×${mm(d.y)}×${mm(d.z)}` });
  }
});
viz.sort((p, q) => q.faces - p.faces);
out.push([`3 · vizinhança do estepe (caixa + 250 mm, +450 em y) — ${viz.length}`,
  '\n        ' + viz.slice(0, 30).map((it) => `${it.cabe ? 'cabe      ' : 'atravessa '}${it.nome} ${it.rot} f=${it.faces}`
    + ` x ${mm(it.box.min.x)}…${mm(it.box.max.x)} y ${mm(it.box.min.y)}…${mm(it.box.max.y)}`
    + ` z ${mm(it.box.min.z)}…${mm(it.box.max.z)}`).join('\n        ')]);
/* 4 · foto do estepe, pelo flanco x−. */
...
S.camera.position.copy(P(-3.6, 1.05, -4.6));
S.controls.target.copy(P(-0.4, 0.55, -5.9));
S.controls.update();
for (let i = 0; i < 14; i++) await B.frame();
out.push(['vm-estepe', S.renderer.domElement.toDataURL('image/png')]);
return out;
