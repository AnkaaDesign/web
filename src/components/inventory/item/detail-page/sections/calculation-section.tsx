...
import { IconClipboardList, IconHelpCircle, IconCalculator, IconTrendingUp, IconTrendingDown, IconMinus, IconShoppingCart } from "@tabler/icons-react";
import type { Item } from "../../../../../types";
import { ABC_CATEGORY_LABELS, XYZ_CATEGORY_LABELS } from "../../../../../constants";
import { formatDate } from "../../../../../utils";
import { cn } from "@/lib/utils";

export interface ScheduledNextOrder {
  quantity: number;
...
function formatNumber(value: number | null | undefined, suffix = ""): string {
  ...
  const formatted = value.toLocaleString("pt-BR", { maximumFractionDigits: 2, minimumFractionDigits: 2 });
...
function formatTrend(trend: number | null | undefined) {
  if (trend == null || !Number.isFinite(trend)) {
    return { label: "Estável", symbol: "→", icon: IconMinus, color: "text-muted-foreground" };
  }
  if (trend > 1) {
    return { label: `+${trend.toFixed(0)}%`, symbol: "↑", icon: IconTrendingUp, color: "text-orange-500" };
  }
  if (trend < -1) {
    return { label: `${trend.toFixed(0)}%`, symbol: "↓", icon: IconTrendingDown, color: "text-blue-500" };
  }
  return { label: "Estável", symbol: "→", icon: IconMinus, color: "text-muted-foreground" };
}

/** Body of the legacy CalculationBreakdownCard (without the Card chrome) — read-only,
 *  computed by the API. The DetailPage section provides the title. */
export function CalculationSection({ item, scheduledNextOrder }: { item: Item; scheduledNextOrder?: ScheduledNextOrder | null }) {
  ...
  const abcXyzKey = item.abcCategory && item.xyzCategory ? `${item.abcCategory}-${item.xyzCategory}` : null;
  const abcXyzExplanation = abcXyzKey ? ABC_XYZ_EXPLANATIONS[abcXyzKey] : null;

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-2 flex-wrap">
        <p className="text-sm text-muted-foreground">Valores calculados automaticamente a partir do consumo e do tempo de entrega.</p>
        {item.hasActiveOrder && (
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">Valores observados que alimentam o cálculo.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-3 rounded-lg bg-muted/50 border-0">
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-foreground">{formatNumber(item.monthlyConsumption, "/ mês")}</span>
          ...
          <div className="p-3 rounded-lg bg-muted/50 border-0">
            ...
            <div className="font-semibold text-foreground">{item.estimatedLeadTime != null ? `${item.estimatedLeadTime} dias` : "—"}</div>
          </div>
            ...
            <div className="font-semibold text-foreground">
              {item.abcCategory ? (
                <div className="flex items-center gap-2">
                  ...
                  {item.abcCategoryOrder != null && <span className="text-xs text-muted-foreground">#{item.abcCategoryOrder}</span>}
                </div>
              ) : (
                "—"
              )}
            ...
            <div className="font-semibold text-foreground">
              {item.xyzCategory ? (
                <div className="flex items-center gap-2">
                  ...
                  {item.xyzCategoryOrder != null && <span className="text-xs text-muted-foreground">#{item.xyzCategoryOrder}</span>}
                </div>
              ) : (
                "—"
              )}
      ...
      <div className="pt-6 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground mb-3">Multiplicadores que ajustam o cálculo conforme criticidade e sazonalidade.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="p-3 rounded-lg bg-muted/50 border-0">
            <div className="text-xs text-muted-foreground mb-1">Fator de segurança</div>
            <div className="font-semibold text-foreground">{formatFactor(safetyFactor)}</div>
          </div>
          <div className="p-3 rounded-lg bg-muted/50 border-0">
            <div className="text-xs text-muted-foreground mb-1">Fator sazonal</div>
            <div className="font-semibold text-foreground">{formatFactor(seasonalFactor)}</div>
          </div>
          <div className="p-3 rounded-lg bg-muted/50 border-0">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button type="button" className="inline-flex" onClick={(e) => e.stopPropagation()} aria-label="Explicação ABC/XYZ">
            ...
            <div className="font-semibold text-foreground">{abcXyzKey ?? "—"}</div>
          </div>
      ...
      <div className="pt-6 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground mb-3">Resultado da fórmula com os dados acima.</p>
        <div className="space-y-3">
          <div className="bg-muted/50 rounded-lg px-4 py-3 space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-base text-foreground">{formatNumber(item.reorderPoint)}</span>
            </div>
            <div className="text-xs text-muted-foreground font-mono">{reorderFormulaText}</div>
          </div>
          <div className="bg-muted/50 rounded-lg px-4 py-3 space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Quantidade máxima</span>
              <span className="font-semibold text-base text-foreground">{formatNumber(item.maxQuantity)}</span>
            </div>
            <div className="text-xs text-muted-foreground font-mono">{maxFormulaText}</div>
          </div>
          {scheduledNextOrder ? (
            <div className="flex items-center justify-between gap-4 bg-primary/5 rounded-lg px-4 py-3">
              ...
              <span className="font-semibold text-base text-primary whitespace-nowrap">{formatNumber(scheduledNextOrder.quantity)}</span>
            </div>
          ) : (
            <div className="flex items-center justify-between bg-primary/5 rounded-lg px-4 py-3">
              <span className="text-sm font-medium text-muted-foreground">Quantidade sugerida para o próximo pedido</span>
              <span className="font-semibold text-base text-primary">{formatNumber(item.reorderQuantity)}</span>
            </div>
          )}
        </div>
      </div>
