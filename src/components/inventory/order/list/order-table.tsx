...
import { IconChevronUp, IconChevronDown, IconEdit, IconTrash, IconSelector, IconExternalLink, IconAlertTriangle, IconShoppingCart, IconCheck, IconChecks, IconX, IconPlus, IconHourglass, IconCircleCheck } from "@tabler/icons-react";
...
export function OrderTable({ visibleColumns, className, onEdit, filters = {}, onDataChange }: OrderTableProps) {
  ...
  const { batchDelete, batchMarkAwaitingPaymentAsync, batchMarkPaidAsync } = useOrderBatchMutations();
  ...
  // "Desfazer pagamento" — revert PAID orders back to AWAITING_PAYMENT.
  ...
  // An order is "unpaid/payable" while it has not been marked PAID.
  const showMarkPaid = canManagePayments && contextOrders.some((o) => o.paymentStatus !== ORDER_PAYMENT_STATUS.PAID);
  // "Desfazer pagamento" reverts an already-PAID order back to AWAITING_PAYMENT.
  const showMarkAwaitingPayment = canManagePayments && contextOrders.some((o) => o.paymentStatus === ORDER_PAYMENT_STATUS.PAID);
  const showPaymentSection = showMarkAwaitingPayment || showMarkPaid;
