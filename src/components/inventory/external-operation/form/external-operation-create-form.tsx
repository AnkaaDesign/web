...
export const ExternalOperationCreateForm = () => {
  ...
  // Operation mode (UI-only, not persisted): "WITHDRAWAL" = item withdrawal, "SERVICE" = service billing (implicit CHARGEABLE)
  const [operationMode, setOperationMode] = useState<OperationMode>("WITHDRAWAL");

  // Billing state (CHARGEABLE only)
  const [customerId, setCustomerId] = useState<string | null>(null);
  const [generateInvoice, setGenerateInvoice] = useState(true);
  const [generateBankSlip, setGenerateBankSlip] = useState(true);
  const [paymentConfig, setPaymentConfig] = useState<PaymentConfig | null>(null);
  const [services, setServices] = useState<BillingServiceRow[]>([]);
  ...
  // Mode change: "Serviço" implies CHARGEABLE; "Retirada de Itens" resets to the default RETURNABLE subtype
  const handleModeChange = useCallback(
    (mode: OperationMode) => {
      if (mode === operationMode) return;
      setOperationMode(mode);
      const newType = mode === "SERVICE" ? EXTERNAL_OPERATION_TYPE.CHARGEABLE : EXTERNAL_OPERATION_TYPE.RETURNABLE;
      form.setValue("type", newType, { shouldValidate: true, shouldDirty: true, shouldTouch: true });
      updateType(newType);
    },
    [operationMode, form, updateType],
  );

  // Mutations
  const { createAsync, isLoading: isSubmitting } = useExternalOperationMutations();
  ...
  // Fetch selected billing customer for review display
  ...
  useEffect(() => {
    if (selectedCustomer?.fantasyName && !withdrawerName?.trim()) {
      form.setValue("withdrawerName", selectedCustomer.fantasyName, { shouldValidate: true, shouldDirty: true, shouldTouch: true });
  ...
  const servicesTotal = useMemo(() => (isChargeable ? validServices.reduce((sum, s) => sum + (Number(s.amount) || 0), 0) : 0), [isChargeable, validServices]);
  ...
  const validateCurrentStep = useCallback((): boolean => {
    switch (currentStep) {
      case 1: {
        ...
        if (withdrawalType === EXTERNAL_OPERATION_TYPE.CHARGEABLE && !customerId) {
          toast.error("Cliente é obrigatório para operações cobráveis");
          return false;
        }
        // Customer only applies to CHARGEABLE operations; other types need the withdrawer name
        if (!(withdrawalType === EXTERNAL_OPERATION_TYPE.CHARGEABLE && customerId) && !trimmedName) {
      ...
      case 3: {
        ...
        // Customer only applies to CHARGEABLE operations; other types need the withdrawer name
        if (!(withdrawalType === EXTERNAL_OPERATION_TYPE.CHARGEABLE && customerId) && !finalName) {
          toast.error("Informe um cliente ou quem retirou.");
          return false;
        }
        if (notes && notes.length > 500) {
        ...
        // Check step 2 validations (in service mode items are optional)
        if (operationMode === "SERVICE") {
          if (validServices.length === 0) {
            toast.error("Adicione pelo menos um serviço");
            return false;
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      const submitType = formValues.type || withdrawalType;
      const chargeable = submitType === EXTERNAL_OPERATION_TYPE.CHARGEABLE;
      const trimmedWithdrawerName = (formValues.withdrawerName || withdrawerName || "").trim();
      const trimmedNotes = (formValues.notes || notes || "").trim();

      // Only include keys that are actually set: null/undefined values must be omitted entirely,
      // otherwise the multipart form-data submit serializes them as "" and the API rejects with 400
      const withdrawalData: ExternalOperationCreateFormData = {
        type: submitType,
        items: Array.from(selectedItems).map((itemId: string) => ({
          itemId,
          withdrawedQuantity: Number(quantities[itemId]) || 1,
          price: chargeable ? Number(prices[itemId]) || 0 : undefined,
        })),
      };
      if (trimmedWithdrawerName) {
        withdrawalData.withdrawerName = trimmedWithdrawerName;
      }
      if (trimmedNotes) {
        withdrawalData.notes = trimmedNotes;
      }
      // Billing fields (CHARGEABLE only) — omitted entirely for other types
      if (chargeable) {
        if (customerId) {
          withdrawalData.customerId = customerId;
        }
        withdrawalData.generateInvoice = generateInvoice;
        withdrawalData.generateBankSlip = generateBankSlip;
        if (paymentConfig) {
          withdrawalData.paymentConfig = paymentConfig;
        }
        if (validServices.length > 0) {
          withdrawalData.services = validServices.map((service, index) => ({
      ...
      const parseResult = externalOperationCreateSchema.safeParse(withdrawalData);
      ...
      if (result.success && result.data) {
        ...
        setTimeout(() => {
          if (result.data?.id) {
            navigate(routes.inventory.externalOperations.details(result.data.id));
          } else {
            // Fallback to list if no ID is returned
            navigate(routes.inventory.externalOperations.root);
          }
        }, 1500);
      }
    } catch (error) {
  ...
  }, [validateCurrentStep, withdrawerName, withdrawalType, notes, selectedItems, quantities, prices, createAsync, form, clearAllSelections, navigate, receiptFiles, invoiceFiles, customerId, generateInvoice, generateBankSlip, paymentConfig, validServices]);
      ...
      <head>
        ...
        <style>
          ...
          * { 
          ...
          ${
            withdrawalType === EXTERNAL_OPERATION_TYPE.CHARGEABLE
              ? `
  ...
  if (!isLastStep) {
    ...
    navigationActions.push({
      ...
      variant: "default" as const,
      disabled: isSubmitting || (operationMode === "SERVICE" ? validServices.length === 0 : selectionCount === 0),
    ...
    <div className="h-full flex flex-col gap-4 bg-background px-4 pt-4 pb-4">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: "Operações Externas", href: routes.inventory.externalOperations?.root || "/inventory/external-operations" },
          { label: "Cadastrar" },
        ]}
              ...
              <div className={cn("flex-1 min-h-0", currentStep === 2 ? "flex flex-col overflow-hidden" : "overflow-y-auto")}>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          {/* Withdrawal Type (withdrawal mode only) */}
                          {operationMode === "WITHDRAWAL" && (
                            <div className="space-y-2">
                              <Label className="text-sm font-medium">
                                Tipo de Operação <span className="text-destructive">*</span>
                          ...
                          {/* Withdrawer Name (optional — the API requires a customer OR the withdrawer name) */}
                          <div className="space-y-2">
                ...
                {currentStep === 2 && (
                  <ItemSelectorTable
                    ...
                    storageKey="external-operation-item-selector"
                ...
                {currentStep === 3 && (
                  <div className="space-y-6">
                          ...
                          {isChargeable && (
                            <>
                              <div>
                                ...
                                <p className="mt-1 font-medium">{selectedCustomer?.fantasyName || "Não informado"}</p>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <p className="mt-1 font-medium">
                                  {validServices.length > 0 ? `${validServices.length} ${validServices.length === 1 ? "serviço" : "serviços"}` : "Nenhum"}
                                </p>
                              </div>
                            </>
                          )}
                    ...
                    {/* Services Table (CHARGEABLE only) */}
                    {isChargeable && validServices.length > 0 && (
                      <Card>
                        <CardHeader>
                          <CardTitle className="flex items-center gap-2">
                            ...
                            <Table>
                              <TableHeader>
                                <TableRow className="bg-muted/50">
                                  ...
                                  {canViewPrices && <TableHead className="text-right font-semibold">Valor</TableHead>}
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                {validServices.map((service, index) => (
                                  <TableRow key={index} className={cn("transition-colors", index % 2 === 1 && "bg-muted/10")}>
                                    ...
                                    {canViewPrices && <TableCell className="text-right font-semibold">{formatCurrency(Number(service.amount) || 0)}</TableCell>}
                                  </TableRow>
                                ))}
                                {canViewPrices && (
                                  <TableRow className="bg-muted/30 font-semibold">
                                    <TableCell className="text-right">Total de Serviços:</TableCell>
                                    <TableCell className="text-right">{formatCurrency(servicesTotal)}</TableCell>
                                  </TableRow>
                                )}
                              </TableBody>
                            </Table>
