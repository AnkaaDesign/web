...
import { IconSelector, IconChevronUp, IconChevronDown, IconAlertTriangle, IconTag, IconEdit, IconTrash } from "@tabler/icons-react";
...
interface BrandTableProps {
  ...
  className?: string;
  onEdit?: (brands: ItemBrand[]) => void;
  onDelete?: (brands: ItemBrand[]) => void;
  filters?: Partial<ItemBrandGetManyFormData>;
  onDataChange?: (data: { brands: ItemBrand[]; totalRecords: number }) => void;
}
...
export function BrandTable({ columns: allColumns, visibleColumns, className, onEdit, onDelete, filters = {}, onDataChange }: BrandTableProps) {
  ...
  // Use the brands hook
  const { data: response, isLoading, error } = useItemBrands(queryParams);
  ...
  React.useEffect(() => {
    if (onDataChange && isMountedRef.current) {
      const dataKey = brands.length > 0 ? `${totalRecords}-${brands.map((brand) => brand.id).join(",")}` : `empty-${totalRecords}`;

      if (dataKey !== lastNotifiedDataRef.current) {
        lastNotifiedDataRef.current = dataKey;
        onDataChange({ brands, totalRecords });
      }
    }
  }, [brands, totalRecords, onDataChange]);
  ...
  // Get current page brand IDs for selection
  const currentPageBrandIds = React.useMemo(() => {
    return brands.map((brand) => brand.id);
  }, [brands]);

  // Selection handlers
  const allSelected = isAllSelected(currentPageBrandIds);
  const partiallySelected = isPartiallySelected(currentPageBrandIds);
  ...
  const handleContextMenu = (e: React.MouseEvent, brand: ItemBrand) => {
    e.preventDefault();
    e.stopPropagation();

    const isBrandSelected = isSelected(brand.id);
    const hasSelection = selectionCount > 0;

    if (hasSelection && isBrandSelected) {
      const selectedBrandsList = brands.filter((b) => isSelected(b.id));
      setContextMenu({
        x: e.clientX,
        y: e.clientY,
  ...
  const handleEdit = () => {
    if (contextMenu) {
      if (contextMenu.isBulk && contextMenu.brands.length > 1) {
        if (onEdit) {
          onEdit(contextMenu.brands);
        } else {
          toast.error("Edição em lote não implementada");
        }
      } else {
        navigate(routes.inventory.products.brands.edit(contextMenu.brands[0].id));
      }
      setContextMenu(null);
    }
  };
  ...
  return (
    <div className={cn("rounded-lg flex flex-col", className)}>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden border-l border-r border-border min-h-0">
            ...
            {error ? (
              <TableRow>
                <TableCell colSpan={columns.length + 1} className="p-0">
                  <div className="flex flex-col items-center justify-center p-8 text-center text-destructive">
                    ...
                    <div className="text-lg font-medium mb-2">Não foi possível carregar as marcas</div>
              ...
              <TableRow>
                <TableCell colSpan={columns.length + 1} className="p-0">
                  <div className="flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
                    ...
                    <div className="text-lg font-medium mb-2">Nenhuma marca encontrada</div>
              ...
              brands.map((brand, index) => {
                ...
                return (
                  <TableRow
                    key={brand.id}
                    data-state={brandIsSelected ? "selected" : undefined}
                    className={cn(
                      "cursor-pointer transition-colors border-b border-border",
                      index % 2 === 1 && "bg-muted/10",
                      "hover:bg-muted/20",
                      brandIsSelected && "bg-muted/30 hover:bg-muted/40",
                    )}
                    onClick={() => navigate(routes.inventory.products.brands.details(brand.id))}
                  ...
                  >
                    ...
                    {columns.map((column) => (
                      <TableCell key={column.key} className={cn(column.className, "p-0 !border-r-0")}>
                        ...
                        >
                          {column.accessor(brand, navigate)}
                        </div>
                      </TableCell>
                    ))}
                  </TableRow>
