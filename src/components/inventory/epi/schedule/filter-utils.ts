...
export function extractActiveFilters(
  ...
  if (filters.nextRunRange?.gte || filters.nextRunRange?.lte) {
    const startDate = filters.nextRunRange.gte ? new Date(filters.nextRunRange.gte).toLocaleDateString("pt-BR") : "";
    const endDate = filters.nextRunRange.lte ? new Date(filters.nextRunRange.lte).toLocaleDateString("pt-BR") : "";
...
export function createFilterRemover(setFilters: React.Dispatch<React.SetStateAction<Partial<PpeDeliveryScheduleGetManyFormData>>>) {
