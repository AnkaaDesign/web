...
function isEmptyItem(item: any, fieldName: string): boolean {
  ...
  // For pricing.services: empty if no description (amount can be 0 for courtesy)
  if (fieldName === 'pricing.services') {
    return !item.description || item.description.trim() === '';
  }
...
export function useEditForm<TFieldValues extends FieldValues = FieldValues, TContext = any, TApiData = any>({
  ...
  const getChangedFields = useCallback(() => {
    ...
    allKeys.forEach((key) => {
      ...
      // Special handling for pricing - check nested services
      if (key === 'pricing') {
        const currentPricing = currentValue as any;
        const originalPricing = originalValue as any;

        if (currentPricing && originalPricing) {
          // Strip auto-generated ids and filter empty items from pricing.services
          const currentServices = currentPricing.services || [];
          const originalServices = originalPricing.services || [];

          const filteredCurrentServices = filterEmptyItems(stripAutoGeneratedFields(currentServices), 'pricing.services');
          const filteredOriginalServices = filterEmptyItems(stripAutoGeneratedFields(originalServices), 'pricing.services');

          // Compare the rest of pricing fields
          const strippedCurrent = { ...currentPricing, services: filteredCurrentServices };
          const strippedOriginal = { ...originalPricing, services: filteredOriginalServices };
          ...
          if (!_.isEqual(strippedCurrent, strippedOriginal)) {
            ...
            if (filteredCurrentServices.length > 0 || filteredOriginalServices.length > 0) {
              changedFields[typedKey] = { ...currentPricing, services: filteredCurrentServices } as any;
            }
          }
