...
interface CpfInputProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "onChange" | "value" | "type"> {
  value?: string;
  onChange?: (value: string | undefined) => void;
  onBlur?: (e: React.FocusEvent<HTMLInputElement>) => void;
}

export function CPFInput({ value, onChange, onBlur, className, placeholder = "000.000.000-00", ...props }: CpfInputProps) {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Backspace") {
        ...
        const formatted = formatCpf(newNumbers);
        setDisplayValue(formatted);
