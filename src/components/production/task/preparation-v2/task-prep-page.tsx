...
// no truck layouts, no nested customerConfigs. `assignedToId` is the one extra scalar we keep so the
// progress cell can flag "pending assigned to you".
const LIST_INCLUDE = {
  ...
  const rowActions = useMemo<DataTableRowAction<ClusteredTask>[]>(() => {
    ...
    if (canEdit) {
      actions.push({
        ...
        onClick: (rows) => rows[0] && navigate(routes.production.preparation.edit(rows[0].id)),
      });
    }
    if (canLiberar) {
      actions.push({
