...
function isEmptyItem(item: any, fieldName: string): boolean {
  ...
  // For quote.services: empty if no description (amount can be 0 for courtesy)
  if (fieldName === 'quote.services') {
    return !item.description || item.description.trim() === '';
  }
...
export function useEditForm<TFieldValues extends FieldValues = FieldValues, TContext = any, TApiData = any>({
  ...
  const getChangedFields = useCallback(() => {
    ...
    allKeys.forEach((key) => {
      ...
      // Special handling for quote - check nested services
      if (key === 'quote') {
        const currentQuote = currentValue as any;
        const originalQuote = originalValue as any;

        if (currentQuote && originalQuote) {
          // Strip auto-generated ids and filter empty items from quote.services
          const currentServices = currentQuote.services || [];
          const originalServices = originalQuote.services || [];

          const filteredCurrentServices = filterEmptyItems(stripAutoGeneratedFields(currentServices), 'quote.services');
          const filteredOriginalServices = filterEmptyItems(stripAutoGeneratedFields(originalServices), 'quote.services');

          // Compare the rest of quote fields
        ...
        } else if (currentQuote !== originalQuote) {
