...
export const ImplementMeasureForm = ({
  selectedSide = 'left',
  layouts,
  layout,
  onChange,
        ...
        sections.forEach((section: any, idx: number) => {
          if (section.isDoor) {
            extractedDoors.push({
              ...
              doorHeight: (section.doorHeight || 1) * 100
            });
          }
  ...
  useEffect(() => {
    ...
    // Don't reset state if backend returns incomplete data (sections: null)
    // This happens when the backend doesn't include sections in the response
    if (layout.sections === null || layout.sections === undefined) {
      return;
    }
      ...
      if (onChange) {
        ...
        if (updates.height !== undefined && selectedSide !== 'back') {
          ...
          const oppositeLayoutData = {
            ...
            sections: oppositeLayoutSections,
            photoId: null,
            photoFile: null,
          };
  ...
  const addDoor = useCallback((e?: React.MouseEvent) => {
    ...
    const doorWidth = 100; // Default door width in cm (1m)
    const defaultDoorHeight = 100; // Default door height from bottom in cm (1m)
