...
//
// Heights are ESTIMATED (field count + per-card chrome + a flat allowance for embedded render content).
// The estimate only decides placement; it never affects the rendered height.
...
const RENDER_ALLOWANCE = 260; // flat allowance for a section with embedded render content
...
function estHeight(s: EstSection): number {
  ...
  if (s.def.render) body += RENDER_ALLOWANCE;
