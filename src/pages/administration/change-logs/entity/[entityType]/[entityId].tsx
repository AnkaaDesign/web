...
import { IconHistory, IconArrowLeft, IconSearch } from "@tabler/icons-react";
...
export function ChangeLogEntityDetails() {
  const { entityType: urlEntityType, entityId: urlEntityId } = useParams<{ entityType: string; entityId: string }>();
  const navigate = useNavigate();

  // Check if params are placeholders or invalid
  const hasValidParams = urlEntityType && urlEntityId && !urlEntityType.startsWith(":") && !urlEntityId.startsWith(":");

  // State for entity selector
  const [selectedEntityType, setSelectedEntityType] = useState<string>(hasValidParams ? urlEntityType : "");
  const [selectedEntityId, setSelectedEntityId] = useState<string>(hasValidParams ? urlEntityId : "");
  const [showSelector, setShowSelector] = useState<boolean>(!hasValidParams);
  ...
  // If no valid params or showing selector
  if (showSelector || !hasValidParams) {
    return (
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Escolha o tipo de entidade e depois selecione a entidade específica para visualizar seu histórico de alterações.
                    </p>
                  </div>
  ...
  // Current entity type and ID from URL
  const entityType = urlEntityType;
  const entityId = urlEntityId;
  const entityLabel = CHANGE_LOG_ENTITY_TYPE_LABELS[entityType as CHANGE_LOG_ENTITY_TYPE] || entityType;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-shrink-0">
        <PageHeader
          ...
          actions={[
            {
              key: "select-entity",
              label: "Selecionar Outra Entidade",
              icon: IconSearch,
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <ChangelogHistory
          entityType={entityType as CHANGE_LOG_ENTITY_TYPE}
          entityId={entityId}
