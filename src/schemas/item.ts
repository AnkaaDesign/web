...
const itemTransform = (data: any) => {
  ...
  if (data.brandIds && Array.isArray(data.brandIds) && data.brandIds.length > 0) {
    ...
    if (hasNullBrand && actualBrandIds.length > 0) {
      // Include both no-brand and specific brands
      andConditions.push({
...
export const itemCreateSchemaBase = z.object({
  ...
  targetCoverageDays: z.number().int("Cobertura deve ser um número inteiro de dias").positive("Cobertura deve ser positiva").nullable().optional(),
  minStockQuantity: z.number().min(0, "Estoque mínimo não pode ser negativo").nullable().optional(),
...
export const mapItemToFormData = createMapToFormDataHelper<Item, ItemUpdateFormData>((item) => ({
  ...
  minStockQuantity: item.minStockQuantity ?? undefined,
