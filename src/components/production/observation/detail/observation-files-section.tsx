...
interface ObservationFilesSectionProps {
  files: AnkaaFile[];
  /** Icon shown in the empty state. */
  emptyIcon: ComponentType<{ className?: string }>;
  emptyTitle: string;
  emptyDescription: string;
  /** Initial layout — "grid" for images, "list" for documents. */
  defaultViewMode?: FileViewMode;
}

/**
 * Bare file gallery — the BODY of the observation "Arquivos Anexados" card without any Card/header
 * chrome (the DetailPage section provides the title). Grid/list toggle + click-to-preview via the
 * app-level file viewer. Mirrors airbrushing-files-section.tsx. FileItem generates its own
 * serve/thumbnail URLs from the raw file record, so raw `observation.files` can be passed directly.
 */
export function ObservationFilesSection({ files, emptyIcon: EmptyIcon, emptyTitle, emptyDescription, defaultViewMode = "grid" }: ObservationFilesSectionProps) {
