...
const Card = React.forwardRef<HTMLDivElement, CardProps>(({ className, nested, level, ...props }, ref) => {
  ...
  return <div ref={ref} className={cn("text-card-foreground rounded-xl shadow-sm border border-border dark:border-border/40 transition-colors", getCardLevel(cardLevel), className)} {...props} />;
});
