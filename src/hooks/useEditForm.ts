 ...
 * - 'id' fields added by useFieldArray (auto-generated for React keys)
...
function stripAutoGeneratedFields(value: any): any {
  ...
  if (value && typeof value === 'object' && !(value instanceof Date) && !(value instanceof File)) {
    ...
    for (const key of Object.keys(value)) {
      // Skip 'id' field entirely for array items - IDs are for tracking, not content comparison
      // The actual entity ID from database will be compared via other fields
      if (key === 'id') {
        continue;
      }
...
function isEmptyItem(item: any, fieldName: string): boolean {
  ...
  // For serviceOrders or services: empty if no description
  if (fieldName === 'serviceOrders' || fieldName === 'services') {
...
export function useEditForm<TFieldValues extends FieldValues = FieldValues, TContext = any, TApiData = any>({
  ...
  const arrayFieldsWithEmptyFiltering = ['services', 'serviceOrders', 'cuts', 'airbrushings'];
