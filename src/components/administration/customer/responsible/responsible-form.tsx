...
export function ResponsibleForm({
  ...
  const form = useForm<ResponsibleFormData>({
    ...
    defaultValues: {
      ...
      roles: initialData?.roles?.length ? initialData.roles : [ResponsibleRole.COMMERCIAL],
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormLabel>Funções <span className="text-destructive">*</span></FormLabel>
                      <FormControl>
                        {/* Multi-select: a contact can handle several areas, and
                            colleagues need to see which ones before calling. */}
                        <Combobox
                          mode="multiple"
                          options={roleOptions}
                          value={(field.value as ResponsibleRole[]) ?? []}
                          onValueChange={value =>
