...
interface ExternalOperationExportProps {
  className?: string;
  currentItems: ExternalOperation[];
  totalRecords: number;
...
const EXPORT_COLUMNS: ExportColumn<ExternalOperation>[] = [
  {
    id: "withdrawerName",
    label: "RESPONSÁVEL",
  ...
  },
  {
    id: "customer",
    label: "CLIENTE",
    getValue: (withdrawal) => withdrawal.customer?.fantasyName ?? withdrawal.withdrawerName ?? "-",
  },
  {
    id: "status",
    label: "STATUS",
    getValue: (withdrawal) => EXTERNAL_OPERATION_STATUS_LABELS[withdrawal.status] || withdrawal.status,
  },
  {
    id: "type",
    label: "TIPO",
    getValue: (withdrawal) => EXTERNAL_OPERATION_TYPE_LABELS[withdrawal.type] || withdrawal.type,
  },
  {
    id: "total",
    label: "VALOR TOTAL",
    getValue: (withdrawal) => {
      if (withdrawal.type === EXTERNAL_OPERATION_TYPE.CHARGEABLE) {
        const itemsTotal = (withdrawal.items || []).reduce((sum, item) => sum + item.withdrawedQuantity * (item.price || 0), 0);
        const servicesTotal = (withdrawal.services || []).reduce((sum, service) => sum + (Number(service.amount) || 0), 0);
        return formatCurrency(itemsTotal + servicesTotal);
      }
      return "-";
    },
  },
...
const DEFAULT_VISIBLE_COLUMNS = new Set(["withdrawerName", "customer", "status", "type", "total", "createdAt"]);

export function ExternalOperationExport({ className, currentItems, totalRecords, selectedItems, visibleColumns, filters }: ExternalOperationExportProps) {
  ...
  const fetchAllItems = async (): Promise<ExternalOperation[]> => {
    const response = await externalOperationService.getExternalOperations({
      ...filters,
      limit: totalRecords,
      page: 1,
      include: {
  ...
  const handleExport = async (format: "csv" | "excel" | "pdf", items: ExternalOperation[], columns: ExportColumn<ExternalOperation>[]) => {
    let content = "";
    let filename = `operacoes_externas_${new Date().toISOString().split("T")[0]}`;
  ...
  const generateCSV = (data: ExternalOperation[], columns: ExportColumn<ExternalOperation>[]) => {
  ...
  const generateExcel = (data: ExternalOperation[], columns: ExportColumn<ExternalOperation>[]) => {
    const headers = columns.map((col) => col.label).join("\t");
  ...
  const generatePDF = (data: ExternalOperation[], columns: ExportColumn<ExternalOperation>[]) => {
    ...
    const tableRows = data.map((withdrawal) => {
      const cells = columns.map((col) => {
        ...
        // Special styling for status (keyed by enum value, not display label)
        if (col.id === "status") {
          const statusColors: Record<EXTERNAL_OPERATION_STATUS, string> = {
            [EXTERNAL_OPERATION_STATUS.PENDING]: "background-color: #3b82f6; color: white;",
            [EXTERNAL_OPERATION_STATUS.PARTIALLY_RETURNED]: "background-color: #eab308; color: white;",
            [EXTERNAL_OPERATION_STATUS.FULLY_RETURNED]: "background-color: #22c55e; color: white;",
            [EXTERNAL_OPERATION_STATUS.CHARGED]: "background-color: #a855f7; color: white;",
            [EXTERNAL_OPERATION_STATUS.LIQUIDATED]: "background-color: #16a34a; color: white;",
            [EXTERNAL_OPERATION_STATUS.DELIVERED]: "background-color: #0ea5e9; color: white;",
            [EXTERNAL_OPERATION_STATUS.CANCELLED]: "background-color: #ef4444; color: white;",
          };
          style = statusColors[withdrawal.status] || "";
        }
    ...
    const pdfContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Operações Externas - Exportação</title>
        <style>
      ...
      <body>
        ...
        <div class="footer">
          Sistema de Gestão - Operações Externas
        </div>
      </body>
      </html>
    `;
  ...
  return (
    <BaseExportPopover<ExternalOperation>
      className={className}
      ...
      entityName="operação externa"
      entityNamePlural="operações externas"
    />
  );
}
