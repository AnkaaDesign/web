...
import { IconFilter, IconX, IconCheck, IconBuilding, IconBriefcase, IconUserCheck, IconUserMinus } from "@tabler/icons-react";
...
export function PayrollFilters({ open, onOpenChange, filters, onApplyFilters }: PayrollFiltersProps) {
  ...
  const { data: usersData } = useUsers({
    ...
    where: {
      isActive: true, // Only active users for payroll
  ...
  const handleSectorsChange = (selectedIds: string[]) => {
    setLocalFilters(prev => ({
      ...
      // Clear include/exclude users when sector changes
      userIds: [],
      excludeUserIds: []
    }));
  };
  ...
  const handlePositionsChange = (selectedIds: string[]) => {
    setLocalFilters(prev => ({
      ...
      // Clear include/exclude users when position changes
      userIds: [],
      excludeUserIds: []
    }));
  };
