...
export function ActivityHistoryCard({ item, className, maxHeight = "500px" }: ActivityHistoryCardProps) {
  ...
  const statistics = useMemo(() => {
    ...
    const userMovements = new Map();
    filteredActivities.forEach((activity) => {
      if (activity.user) {
        const count = userMovements.get(activity.user.id) || 0;
        userMovements.set(activity.user.id, count + 1);
      }
    });

    let mostActiveUser = null;
    let maxMovements = 0;
    userMovements.forEach((count, userId) => {
      if (count > maxMovements) {
        maxMovements = count;
        const users = filteredActivities
          .filter((a) => a.user?.id === userId)
          .map((a) => a.user)
          .filter((user, index, arr) => user && arr.findIndex((u) => u?.id === user.id) === index);
        mostActiveUser = users[0] || null;
      }
    });
    ...
    return {
      ...
      totalOut,
      mostActiveUser,
      mostActiveUserMovements: maxMovements,
    };
  }, [filteredActivities]);
              ...
              {groupedActivities.map(([date, dayActivities], groupIndex) => {
                ...
                return (
                  <div key={date} className="relative">
                    {/* Date Header - Same style as changelog history */}
