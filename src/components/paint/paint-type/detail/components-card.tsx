...
export function PaintTypeComponentsCard({ paintType }: PaintTypeComponentsCardProps) {
  const navigate = useNavigate();
  const components = paintType.componentItems || [];
      ...
      <Card className="h-full flex flex-col">
        ...
        <CardContent className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Nenhum componente configurado para este tipo de tinta.</p>
            <p className="text-sm text-muted-foreground mt-2">Configure os componentes editando o tipo de tinta.</p>
          </div>
        </CardContent>
      </Card>
    );
            ...
            <Table className="w-full [&>div]:border-0 [&>div]:rounded-none">
              <TableHeader className="[&_tr]:border-b-0 [&_tr]:hover:bg-muted sticky top-0 z-10">
                <TableRow className="bg-muted hover:bg-muted even:bg-muted">
                  ...
                  <TableHead className="whitespace-nowrap text-foreground font-bold uppercase text-xs p-0 bg-muted !border-r-0">
                    <div className="flex items-center px-4 py-3">Marca</div>
                  </TableHead>
                  <TableHead className="whitespace-nowrap text-foreground font-bold uppercase text-xs p-0 bg-muted !border-r-0">
                ...
                {components.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/estoque/produtos/detalhes/${item.id}`)}>
                      ...
                      <TableCell>{item.brand ? <Badge variant="outline">{item.brand.name}</Badge> : <span className="text-muted-foreground">-</span>}</TableCell>
                      <TableCell>
                        {weightMeasure ? (
