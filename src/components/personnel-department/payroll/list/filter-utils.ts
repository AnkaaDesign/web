// Filter utilities for payroll list - extracting active filters for badge display
...
export function extractActiveFilters(
  ...
  // Exclude User filters (individual badges)
  if (filters.excludeUserIds && filters.excludeUserIds.length > 0) {
    const excludedUsers = users.filter((user) => filters.excludeUserIds?.includes(user.id));
    excludedUsers.forEach((user) => {
      activeFilters.push({
        key: `excludeUserIds-${user.id}`,
        label: "Excluir",
        value: user.name,
        iconType: "user-minus",
        itemId: user.id,
        onRemove: () => onRemoveFilter("excludeUserIds", user.id),
      });
    });
  }
...
export function createFilterRemover(
  currentFilters: PayrollFiltersData,
  defaultFilters: PayrollFiltersData,
  onFilterChange: (filters: PayrollFiltersData) => void
) {
  return (key: string, itemId?: string) => {
    ...
    switch (key) {
      ...
      case "userIds":
        ...
        break;
      case "excludeUserIds":
        if (itemId && Array.isArray(newFilters.excludeUserIds)) {
          const filtered = newFilters.excludeUserIds.filter((id) => id !== itemId);
          newFilters.excludeUserIds = filtered.length > 0 ? filtered : undefined;
        } else {
          newFilters.excludeUserIds = undefined;
        }
        break;
    }
