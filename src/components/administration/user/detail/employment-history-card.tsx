...
interface EmploymentHistoryCardProps {
   ...
   * supplies the single card chrome + "Histórico de Vínculos" title. Default
 ...
 *
 * Read-only: no salary is ever displayed here (it lives on the dedicated
 * Admissões pages), so the card is safe for the collaborator detail audience.
 */
export function EmploymentHistoryCard({ userId, className, maxHeight = "500px", onCount, embedded = false }: EmploymentHistoryCardProps) {
  ...
  // provides the single card chrome + "Histórico de Vínculos" title.
  if (embedded) {
