...
import { createPpeScheduleColumns } from "./ppe-schedule-table-columns";
...
interface PpeScheduleTableProps {
  ...
  onDelete?: (schedules: PpeDeliverySchedule[]) => void;
...
export function PpeScheduleTable({ visibleColumns, className, scheduleRoutes, onEdit, onActivate, onDeactivate, onDelete, filters = {}, onDataChange }: PpeScheduleTableProps) {
  ...
  // Fetch data
  const { data, isLoading, error } = usePpeDeliverySchedules(queryFilters);
  ...
  const columns = createPpeScheduleColumns();
  ...
  const handleContextMenu = (e: React.MouseEvent, schedule: PpeDeliverySchedule) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleActivate = () => {
    if (contextMenu && onActivate) {
      onActivate(contextMenu.schedules);
      setContextMenu(null);
    }
  };
  ...
  const handleDeactivate = () => {
    if (contextMenu && onDeactivate) {
      onDeactivate(contextMenu.schedules);
      setContextMenu(null);
    }
  };
  ...
  return (
    <div className={cn("rounded-lg flex flex-col overflow-hidden", className)}>
      ...
      <DropdownMenu open={!!contextMenu} onOpenChange={(open) => !open && setContextMenu(null)}>
        ...
        >
          {contextMenu?.isBulk && <div className="px-2 py-1.5 text-sm font-semibold text-muted-foreground">{contextMenu.schedules.length} agendamentos selecionados</div>}
          ...
          {contextMenu?.schedules.some((s) => !s.isActive) && (
            <DropdownMenuItem onClick={handleActivate}>
              ...
              {contextMenu?.isBulk && contextMenu.schedules.length > 1 ? "Ativar agendamentos" : "Ativar agendamento"}
            </DropdownMenuItem>
          )}
          ...
          {contextMenu?.schedules.some((s) => s.isActive) && (
            <DropdownMenuItem onClick={handleDeactivate}>
              ...
              {contextMenu?.isBulk && contextMenu.schedules.length > 1 ? "Desativar agendamentos" : "Desativar agendamento"}
            </DropdownMenuItem>
          )}
          ...
          <DropdownMenuItem onClick={handleDelete} className="text-destructive">
            ...
            {contextMenu?.isBulk && contextMenu.schedules.length > 1 ? "Deletar selecionados" : "Deletar"}
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
