...
export const FaltasListPage = () => {
  const navigate = useNavigate();
  const [createOpen, setCreateOpen] = useState(false);
  usePageTracker({ title: "Faltas", icon: "userX" });
    ...
    <PrivilegeRoute requiredPrivilege={[SECTOR_PRIVILEGES.HUMAN_RESOURCES, SECTOR_PRIVILEGES.ADMIN]}>
      <div className="h-full flex flex-col gap-4 bg-background px-4 pt-4">
        ...
        <div className="flex-1 min-h-0 pb-6 flex flex-col">
          <AbsenceList category="FALTA" className="h-full" />
        </div>
        <AbsenceFormDialog open={createOpen} onOpenChange={setCreateOpen} category="FALTA" />
      </div>
    </PrivilegeRoute>
  );
};

export default FaltasListPage;
