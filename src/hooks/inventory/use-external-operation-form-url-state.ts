...
export type ExternalOperationFormStage = 1 | 2 | 3;

export interface ExternalOperationFormValidationState {
  stage1Valid: boolean;
  stage2Valid: boolean;
...
interface UseExternalOperationFormUrlStateOptions {
...
const externalOperationFormFilterConfig = {
  // Stage navigation
  stage: {
    schema: z.coerce.number().int().min(1).max(3).default(1),
    defaultValue: 1 as ExternalOperationFormStage,
  ...
  type: {
    schema: z.nativeEnum(EXTERNAL_OPERATION_TYPE).default(EXTERNAL_OPERATION_TYPE.RETURNABLE),
    defaultValue: EXTERNAL_OPERATION_TYPE.RETURNABLE,
    debounceMs: 0,
  },
  notes: {
    schema: z.string().default(""),
...
export function useExternalOperationFormUrlState(options: UseExternalOperationFormUrlStateOptions = {}) {
  ...
  // EDIT mode (initialData provided): defaults are the saved entity values, so collapsing a field
  // back to the create-default (e.g. "" or RETURNABLE) by writing `undefined` would silently revert
  // the user's change to the saved value. In edit mode always write explicit values to URL state.
  const isEditMode = !!initialData;

  // Update externalOperationFormFilterConfig with custom defaults and initial data
  const configWithDefaults = useMemo(() => {
    const config = { ...externalOperationFormFilterConfig };
  ...
  const stage = (filters.stage || 1) as ExternalOperationFormStage;
  const quantities = filters.quantities || {};
  const prices = filters.prices || {};
  const withdrawerName = filters.withdrawerName || "";
  const type = filters.type !== undefined ? filters.type : EXTERNAL_OPERATION_TYPE.RETURNABLE;
  ...
  const updateSelectedItems = useCallback(
    (newSelected: Set<string>) => {
      ...
      setFilter("selectedItems", isEditMode ? selectedArray : selectedArray.length > 0 ? selectedArray : undefined);
    },
    [setFilter, isEditMode],
  );
  ...
  const updateQuantities = useCallback(
    (newQuantities: Record<string, number>) => {
      ...
      setFilter("quantities", isEditMode ? filtered : Object.keys(filtered).length > 0 ? filtered : undefined);
    },
    [setFilter, isEditMode],
  );
  ...
  const updatePrices = useCallback(
    (newPrices: Record<string, number>) => {
      ...
      setFilter("prices", isEditMode ? filtered : Object.keys(filtered).length > 0 ? filtered : undefined);
    },
    [setFilter, isEditMode],
  );
  ...
  const updateWithdrawerName = useCallback(
    (name: string) => {
      setFilter("withdrawerName", isEditMode ? name : name === "" ? undefined : name);
    },
    [setFilter, isEditMode],
  );

  const updateType = useCallback(
    (value: EXTERNAL_OPERATION_TYPE) => {
      setFilter("type", isEditMode ? value : value === EXTERNAL_OPERATION_TYPE.RETURNABLE ? undefined : value);
    },
    [setFilter, isEditMode],
  );
  ...
  const updateNotes = useCallback(
    (text: string) => {
      setFilter("notes", isEditMode ? text : text === "" ? undefined : text);
    },
    [setFilter, isEditMode],
  );
  ...
  const setStage = useCallback(
    (newStage: ExternalOperationFormStage) => {
  ...
  const validation = useMemo((): ExternalOperationFormValidationState => {
    const errors: ExternalOperationFormValidationState["errors"] = {};
  ...
  const goToNextStage = useCallback(() => {
    if (stage < 3) {
      const nextStage = (stage + 1) as ExternalOperationFormStage;
  ...
  const goToPrevStage = useCallback(() => {
    if (stage > 1) {
      const prevStage = (stage - 1) as ExternalOperationFormStage;
      setStage(prevStage);
      return true;
    }
    return false;
  ...
  const goToStage = useCallback(
    (targetStage: ExternalOperationFormStage) => {
  ...
  const toggleItemSelection = useCallback(
    (itemId: string, quantity?: number, price?: number) => {
      ...
      if (hasChanges) {
        // In edit mode always write explicit values so empty selections don't revert to the saved defaults
        setFilters({
          selectedItems: isEditMode || newSelected.size > 0 ? Array.from(newSelected) : undefined,
          quantities: isEditMode || Object.keys(newQuantities).length > 0 ? newQuantities : undefined,
          prices: isEditMode || Object.keys(newPrices).length > 0 ? newPrices : undefined,
        });
      }
    },
    [selectedItems, quantities, prices, defaultQuantity, defaultPrice, preserveQuantitiesOnDeselect, setFilters, isEditMode],
  );

  // Helper to clear all selections
  const clearAllSelections = useCallback(() => {
    const resetData: any = {
      selectedItems: isEditMode ? [] : undefined,
    };

    if (!preserveQuantitiesOnDeselect) {
      resetData.quantities = isEditMode ? {} : undefined;
      resetData.prices = isEditMode ? {} : undefined;
    }

    setFilters(resetData);
  }, [preserveQuantitiesOnDeselect, setFilters, isEditMode]);
  ...
  const setItemQuantity = useCallback(
    ...
    [quantities, selectedItems, setFilters, isEditMode],
  );
  ...
  const hasFormData = useMemo(() => {
    return (withdrawerName?.trim() || "") !== "" || type !== EXTERNAL_OPERATION_TYPE.RETURNABLE || (notes?.trim() || "") !== "" || selectedItems.size > 0;
  }, [withdrawerName, type, notes, selectedItems.size]);

  // Check if specific stages have data
  const stageHasData = useMemo(() => {
    return {
      stage1: (withdrawerName?.trim() || "") !== "" || type !== EXTERNAL_OPERATION_TYPE.RETURNABLE || (notes?.trim() || "") !== "",
 ...
 * ```tsx
 * const hook = useExternalOperationFormUrlState({
 *   defaultQuantity: 1,
 *   defaultPrice: 0,
 ...
 * ```tsx
 * const { formProgress, stageHasData, hasFormData } = useExternalOperationFormUrlState();
 *
 * // Show progress bar
