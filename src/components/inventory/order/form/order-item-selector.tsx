...
interface OrderItemSelectorProps {
  ...
  onBatchFiltersChange?: (filters: { showInactive?: boolean; categoryIds?: string[]; brandIds?: string[]; supplierIds?: string[]; resetPage?: boolean }) => void;
  // Sorting props
...
export const OrderItemSelector = ({
  selectedItems,
  onSelectItem,
  onSelectAll: _onSelectAll, // Intentionally unused
  className,
  onQuantityChange,
  onPriceChange,
  onIcmsChange,
  ...
  onSearchTermChange,
  // Filter change handlers are unused in this implementation
  ...
  onBatchFiltersChange: _onBatchFiltersChange, // Intentionally unused
  // Sorting props
                ...
                <DialogContent className="sm:max-w-[600px]">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Combobox<ComboboxOption>
                        ...
                        onValueChange={(value) => setTempCategoryIds(Array.isArray(value) ? value : [])}
                        mode="multiple"
                        className="h-10"
                        searchable={true}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Combobox<ComboboxOption>
                        ...
                        value={tempBrandIds}
                        onValueChange={(value) => setTempBrandIds(Array.isArray(value) ? value : [])}
                        mode="multiple"
                        className="h-10"
                        searchable={true}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Combobox<ComboboxOption>
                        ...
                        onValueChange={(value) => setTempSupplierIds(Array.isArray(value) ? value : [])}
                        mode="multiple"
                        className="h-10"
                        searchable={true}
                      />
                  ...
                  <DialogFooter className="gap-2">
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        // Reset all temp filters
                        setTempShowInactive(false);
                        setTempCategoryIds([]);
                        setTempBrandIds([]);
                        setTempSupplierIds([]);

                        // Use direct URL update for immediate clearing
                        directUpdateFilters({
                          showInactive: false,
                          categoryIds: [],
                          brandIds: [],
                          supplierIds: [],
                          page: 1, // Reset page in same update
                        });
                    ...
                    <Button
                      type="button"
                      onClick={() => {
                        // Use direct URL update for immediate, atomic updates
                        directUpdateFilters({
                    ...
                    >
                      Aplicar filtros
                      {(tempShowInactive || tempCategoryIds.length > 0 || tempBrandIds.length > 0 || tempSupplierIds.length > 0) && (
                        <Badge variant="secondary" className="ml-2">
                          {[tempShowInactive ? 1 : 0, tempCategoryIds.length > 0 ? 1 : 0, tempBrandIds.length > 0 ? 1 : 0, tempSupplierIds.length > 0 ? 1 : 0].reduce(
                            (a, b) => a + b,
                            0,
                          )}
                        </Badge>
                ...
                <TableHead className={cn(TABLE_LAYOUT.checkbox.className, "whitespace-nowrap text-foreground font-bold uppercase text-xs bg-muted !border-r-0 p-0")}>
                  <div className="flex items-center justify-center h-full w-full px-2">
                    <Checkbox
                      ...
                      className={cn("h-4 w-4", someCurrentPageSelected && !allCurrentPageSelected && "data-[state=checked]:bg-muted data-[state=checked]:text-muted-foreground")}
                    />
                  </div>
                </TableHead>
            ...
            <Table className={cn("w-full [&>div]:border-0 [&>div]:rounded-none", TABLE_LAYOUT.tableLayout)}>
              <TableBody>
                {items.map((item, index) => {
                  ...
                  // Safely handle price with multiple fallbacks
                  const storedPrice = prices[item.id];
                  const itemDefaultPrice = item.prices?.[0]?.value;
                  const price = (typeof storedPrice === 'number' && !isNaN(storedPrice))
                    ? storedPrice
