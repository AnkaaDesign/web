...
export const VacationCreatePage = () => {
  ...
  // one (single-period) vacation per person — collective is NOT a separate
  // entity, just multiple regular vacations. Each colaborador's acquisitive
  // period is derived server-side from their own admission.
  const handleSubmit = async (data: VacationCreateSubmit) => {
    const { userIds, ...base } = data;
    if (!userIds.length) return;
    try {
      setIsSubmitting(true);
      const results = await Promise.all(userIds.map((userId) => createAsync({ userId, ...base })));
      if (userIds.length === 1 && results[0]?.data?.id) {
        navigate(routes.personnelDepartment.vacations.details(results[0].data.id));
      } else {
