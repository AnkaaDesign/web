...
export const COPYABLE_FIELD_PERMISSIONS: Record<Exclude<CopyableTaskField, 'all'>, string[]> = {
  ...
  // Prazo de Entrega — espelha o domínio `term` da API (ADMIN + PRODUCTION_MANAGER). O
  // `copy-from` não passa pelo validador de campos do servidor, então esta lista é o freio.
  term: ['ADMIN', 'PRODUCTION_MANAGER'],
