...
// Chave de persistência da visualização do canvas (zoom/pan) em localStorage.
const POSTIT_VIEW_STORAGE_KEY = "ankaa.postits.canvasView";
...
export function PostItsPage() {
  ...
  const [view, setView] = useState<{ scale: number; x: number; y: number }>(() => {
    try {
      const raw = localStorage.getItem(POSTIT_VIEW_STORAGE_KEY);
      if (raw) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(POSTIT_VIEW_STORAGE_KEY, JSON.stringify(view));
    } catch {
      /* ignore */
    }
  }, [view]);
  ...
  // anexamos um listener nativo não-passivo no board.
  useEffect(() => {
  ...
  }, [showArchived, isLoading, postits.length]);
