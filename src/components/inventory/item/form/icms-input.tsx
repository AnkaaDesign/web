...
export function IcmsInput({ control, disabled, required, priceFieldName, onPriceUpdate, watch }: IcmsInputProps) {
  const [taxIncluded, setTaxIncluded] = useState(false);

  const handleTaxIncludedChange = (checked: boolean, currentPrice: number, icmsRate: number) => {
    setTaxIncluded(checked);

    if (checked && currentPrice && icmsRate && onPriceUpdate) {
  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>ICMS {required && <span className="text-destructive">*</span>}</FormLabel>
          <FormControl>
