 ...
 *
 * Só existem DUAS exceções, ambas deliberadas:
 *  - a foto da plaqueta (`FileUploadField variant="inline"`), que precisa caber
 *    numa linha de campos de texto;
 *  - check-in/check-out, onde o volume de fotos por ordem de serviço torna o
 *    card grande impraticável.
 *
 * O seletor de status (DRAFT/APPROVED/REPROVED) NÃO é embutido: só layouts têm
 * esse fluxo, então ele entra por `renderStatus` — ver `LayoutFileUploadField`.
 */
export interface FileCardUploadFieldProps {
  ...
  showStatus?: boolean;
  /**
   * Renderiza o controle de status de UM arquivo. Sem isto (o caso de arquivos base,
   * projetos, evidências) o card não mostra status nenhum.
   */
  renderStatus?: (args: {
    value: string;
...
export function FileCardUploadField({
  onFilesChange,
  onStatusChange,
  showStatus = false,
  renderStatus,
