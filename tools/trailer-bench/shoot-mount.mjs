/* Serve a bancada de MONTAGEM (caminhão rígido + sobrechassi), abre o Chromium
   e salva os PNGs.
   ---------------------------------------------------------------------------
...
const OUT = join(HERE, 'shots-montagem');
/** O caminhão RÍGIDO, relativo a `public/models/trucks/`. */
const TRUCK = process.argv[2] || 'volvo_vm_2015.glb';
/** O percentil da mesa da longarina — ver o cabeçalho de `mountprobe.ts`. */
const PCT = process.argv[3] || '0.75';
/** Folga entre a testeira da carroceria e a traseira da cabine, em metros. */
const GAP = process.argv[4] || '0.15';
/** `?ss=` — ver a nota de superamostragem em `mountprobe.ts`. */
...
const built = await esbuild.build({
  entryPoints: [join(HERE, 'mountprobe.ts')],
...
const HTML = `<!doctype html><meta charset="utf-8"><title>mountcheck</title>
<style>html,body{margin:0;background:#222}canvas{display:block}</style>
<script type="module" src="/mountprobe.js"></script>`;
...
await page.goto(`${url}?truck=${encodeURIComponent(TRUCK)}&pct=${PCT}&gap=${GAP}&ss=${SS}`,
  { waitUntil: 'load' });
...
/* `planos: true` esconde a carroceria e acende as quatro lâminas candidatas —
   é a foto que decide o percentil. As outras mostram o conjunto montado. */
const SHOTS = [
  ['planos-lado', [1, 0.008, 0], [0, 1.1, -3.0], 17, true],
  ['planos-mesa', [1, 0.22, 0.10], [0, 1.1, -3.0], 7.5, true],
  ['planos-perto', [1, 0.16, 0.34], [0, 1.1, -1.6], 3.0, true],
  ['lateral', [1, 0.04, 0], null, 0, false],
  ['3-4-dianteira', [1, 0.32, -0.85], null, 0, false],
  ['3-4-traseira', [-0.95, 0.30, 0.9], null, 0, false],
  ['encontro-cabine', [1, 0.18, -0.55], null, 4.2, false],
  ['traseira', [0, 0.12, 1], null, 0, false],
];
for (const [name, dir, target, dist, planos] of SHOTS) {
  const data = await page.evaluate(
    ([d, t, k, p]) => window.__shot(d, t || null, k, p),
    [dir, target, dist, planos]);
