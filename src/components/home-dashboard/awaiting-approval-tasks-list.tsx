...
interface AwaitingApprovalTasksListProps {
  tasks: HomeDashboardTask[];
}

export function AwaitingApprovalTasksList({ tasks }: AwaitingApprovalTasksListProps) {
  ...
  return (
    <DashboardCardList
      title="Aguardando Aprovação de Pagamento"
      icon={<IconClock className="h-4 w-4 text-amber-500" />}
      viewAllLink="/producao/tarefas"
      emptyMessage="Nenhuma tarefa aguardando aprovação"
