...
export const generateFileUrls = (file: AnkaaFile, baseUrl?: string) => {
  ...
  // Check if file has a URL property (generated by API with external URL)
  const hasExternalUrl = file.url && file.url.startsWith("http");

  // Check if this is a remote storage file (path starts with http)
  const isRemoteFile = file.path && file.path.startsWith("http");

  // For files with external URL (from API) or remote storage files, use the direct URLs
  if (hasExternalUrl || isRemoteFile) {
    const directUrl = hasExternalUrl ? file.url : file.path;
    return {
      serve: directUrl, // Direct external URL
      download: directUrl, // Direct external URL
      thumbnail: file.thumbnailUrl ? (file.thumbnailUrl.startsWith("http") ? file.thumbnailUrl : `${apiUrl}${file.thumbnailUrl}`) : null,
      thumbnailSmall: file.thumbnailUrl || directUrl, // Use thumbnail or file itself
      thumbnailMedium: file.thumbnailUrl || directUrl,
      thumbnailLarge: file.thumbnailUrl || directUrl,
    };
  }
