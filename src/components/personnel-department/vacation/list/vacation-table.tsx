...
import { useVacations } from "../../../../hooks/personnel-department/use-vacations";
...
export function VacationTable({ visibleColumns, className, onDelete, filters = {} }: VacationTableProps) {
  ...
  const { data: response, isLoading, error } = useVacations(queryParams);
  ...
  const allColumns = createVacationColumns();
  ...
  const handleContextMenu = (e: React.MouseEvent, vacation: Vacation) => {
    e.preventDefault();
    e.stopPropagation();
    setContextMenu({ x: e.clientX, y: e.clientY, vacation });
  };

  const handleViewDetails = () => {
    if (contextMenu) {
      navigate(routes.personnelDepartment.vacations.details(contextMenu.vacation.id));
      setContextMenu(null);
    }
  };
  ...
  const handleEdit = () => {
    if (contextMenu) {
      navigate(routes.personnelDepartment.vacations.edit(contextMenu.vacation.id));
      setContextMenu(null);
    }
  };
            ...
            {error ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="p-0">
                  <div className="flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
                    ...
                    <div className="text-lg font-medium mb-2">Nenhum registro de férias encontrado</div>
                    {filters && Object.keys(filters).length > 1 ? (
                      <div className="text-sm">Ajuste os filtros para ver mais resultados.</div>
                    ) : (
                      <div className="text-sm">Use a ação "Novas Férias" para iniciar o primeiro período aquisitivo.</div>
                    )}
                  </div>
                </TableCell>
              </TableRow>
              ...
              vacations.map((vacation, index) => (
                <TableRow
                  ...
                  onClick={() => navigate(routes.personnelDepartment.vacations.details(vacation.id))}
                ...
                >
                  {columns.map((column) => (
                    ...
                    >
                      <div className="px-4 py-2">{column.accessor(vacation)}</div>
                    </TableCell>
                  ))}
                </TableRow>
              ))
