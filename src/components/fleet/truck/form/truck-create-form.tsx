...
import type { TruckCreateFormData } from "../../../../schemas";
import { truckCreateSchema } from "../../../../schemas";
...
interface TruckCreateFormProps {
  ...
  defaultValues?: Partial<TruckCreateFormData>;
}

export function TruckCreateForm({ onSuccess, defaultValues }: TruckCreateFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const navigate = useNavigate();

  const form = useForm<TruckCreateFormData>({
  ...
  const { create } = useTruckMutations();

  const handleSubmit = async (data: TruckCreateFormData) => {
    try {
      ...
      if (result?.success && result.data) {
        ...
        toast.error(result?.message || "Erro ao cadastrar caminhão");
      }
    } catch (error) {
      toast.error("Erro ao cadastrar caminhão. Por favor, tente novamente.");
    } finally {
  ...
  const handleCancel = () => {
    navigate(routes.production.trucks?.list || "/production/trucks");
  };
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="plate" {...field} disabled={isSubmitting} />
                          </FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} placeholder="Ex: Constellation 26.280" disabled={isSubmitting} maxLength={100} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} placeholder="ID da tarefa associada" disabled={isSubmitting} />
                          </FormControl>
