/* TREZE — QUEM DESENHA A LINHA: o filete, o rebite, ou o que sobrou do furo.
   ===========================================================================
   Três candidatos e três fotos, uma variável por foto. E a grade vista do
   ângulo do dono, para medir "muito para trás" em pixel além de em milímetro.

     node tools/studio-bench/bench.mjs --gpu --geometry --checks diag/checks-scania13-0822.mjs */
...
const vf = filetes.map((o) => o.visible), vr = rebites.map((o) => o.visible);
tira('q13-0-como-esta', ...RENTE);
for (const o of filetes) o.visible = false;
tira('q13-1-sem-filete', ...RENTE);
filetes.forEach((o, i) => { o.visible = vf[i]; });
for (const o of rebites) o.visible = false;
tira('q13-2-sem-rebite', ...RENTE);
for (const o of filetes) o.visible = false;
tira('q13-3-sem-nenhum-dos-dois', ...RENTE);
filetes.forEach((o, i) => { o.visible = vf[i]; });
rebites.forEach((o, i) => { o.visible = vr[i]; });

/* ─── A GRADE, do ângulo do dono (traseira, de baixo) ─── */
tira('q13-grade-traseira', 6.5, 35, 6, V(0, -1.1, -2.6));
tira('q13-grade-canto', 3.0, 55, 4, V(0, -1.3, -3.6));
