...
import {
  ...
  SERVICE_ORDER_STATUS as SO_STATUS,
...
export function ServiceOrderCell({
  task,
  serviceOrderType,
  navigationRoute,
}: ServiceOrderCellProps) {
  ...
  const completedCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.COMPLETED,
  ).length;
  const inProgressCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.IN_PROGRESS,
  ).length;
  const pausedCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.PAUSED,
  ).length;
  const waitingApproveCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.WAITING_APPROVE,
  ).length;
  const waitingArtworkCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.WAITING_ARTWORK,
  ).length;
  const cancelledCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.CANCELLED,
  ).length;
  const pendingCount = serviceOrders.filter(
    (so) => so.status === SO_STATUS.PENDING,
  ).length;
          ...
          <Tooltip delayDuration={500}>
            <TooltipTrigger asChild>
              <div className="absolute top-0 right-0 z-[5] w-0 h-0 border-t-[28px] border-l-[28px] border-l-transparent border-t-red-500 pointer-events-auto cursor-help">
                <IconAlertTriangle
                  className="absolute -top-[25px] right-[2px] h-3 w-3 text-white"
            ...
            <TooltipContent side="left" className="max-w-xs">
              <div className="text-sm">
                <div className="font-medium text-red-500">
                  Ordem de serviço ausente
                </div>
                <div className="text-muted-foreground">
                  Esta tarefa não possui nenhuma ordem de serviço de{" "}
  ...
  const waitingArtworkPercent = (waitingArtworkCount / totalCount) * 100;
  const inProgressPercent = (inProgressCount / totalCount) * 100;
  const pausedPercent = (pausedCount / totalCount) * 100;
        ...
        <TooltipTrigger asChild>
          <div className="relative cursor-help">
            ...
            <div className="relative h-5 min-w-[90px] max-w-[140px] bg-gray-200 dark:bg-gray-700 rounded overflow-hidden shadow-sm">
              {/* Status segments, left-to-right with a running offset so the
                    cumulative positions can never desync. Colors mirror the
                    status badges (getServiceOrderStatusColor): WAITING_ARTWORK
                    and WAITING_APPROVE are both purple. */}
              {(() => {
                const segments = [
                  {
                    key: "completed",
                    count: completedCount,
                    pct: completedPercent,
                    className: "bg-green-700",
                  },
                  {
                    key: "waitingApprove",
                    count: waitingApproveCount,
                    pct: waitingApprovePercent,
                    className: "bg-purple-600",
                  },
                  {
                    key: "waitingArtwork",
                    count: waitingArtworkCount,
                    pct: waitingArtworkPercent,
                    className: "bg-purple-600",
                  },
                  {
                    key: "inProgress",
                    count: inProgressCount,
                    pct: inProgressPercent,
        ...
        <TooltipContent side="top" className="max-w-md">
          <div className="text-sm space-y-2">
            <div className="font-medium mb-2">
              Ordens de serviço de {typeLabel.toLowerCase()} ({completedCount}/
              {totalCount})
            </div>
            ...
            <div className="space-y-1.5 max-h-64 overflow-y-auto">
              {serviceOrders.map((serviceOrder, index) => (
                <div
                  key={serviceOrder.id || index}
                  className="flex items-start justify-between gap-2 py-1 border-b border-border/30 last:border-0"
                >
              ...
              <div className="mt-2 pt-2 border-t border-border">
                <div className="flex items-center gap-2 text-red-500 font-medium">
                  <div className="w-0 h-0 border-t-[14px] border-l-[14px] border-l-transparent border-t-red-500 flex items-center justify-center relative">
                    <span className="absolute -top-[11px] right-[1.5px] text-[8px] text-white font-bold">
      ...
      {pendingAssignedCount > 0 && (
        <Tooltip delayDuration={500}>
          <TooltipTrigger asChild>
            <div className="absolute top-0 right-0 z-[5] w-0 h-0 border-t-[28px] border-l-[28px] border-l-transparent border-t-red-500 pointer-events-auto cursor-help">
              <span
                className="absolute -top-[25px] right-[2px] h-3 w-3 flex items-center justify-center text-[11px] font-bold text-white leading-none"
                style={{ filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.3))" }}
              >
                {pendingAssignedCount}
              </span>
          ...
          <TooltipContent side="left" className="max-w-xs">
            <div className="text-sm">
              <div className="font-medium text-red-500">
                Ordens pendentes atribuídas
              </div>
              <div className="text-muted-foreground">
                Você possui {pendingAssignedCount}{" "}
