...
interface ExternalOperationItemSelectorProps {
...
export const ExternalOperationItemSelector = ({
  selectedItems,
  onSelectItem,
  className,
  onQuantityChange,
...
}: ExternalOperationItemSelectorProps) => {
  ...
  const handleSelectAll = useCallback(() => {
    ...
    const filteredIds = currentFiltered.map((item: Item) => item.id);
  ...
  const allSelected = currentPageItemIds.length > 0 && currentPageItemIds.every((id) => selectedItems.has(id));
  const partiallySelected = currentPageItemIds.some((id) => selectedItems.has(id)) && !allSelected;
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Combobox
                      ...
                      options={categories.map((cat: ItemCategory) => ({
                        value: cat.id,
                        label: cat.name,
                      }))}
                  ...
                  <div className="space-y-2">
                    ...
                    <Combobox
                      ...
                      options={brands.map((brand: ItemBrand) => ({
                        value: brand.id,
                        label: brand.name,
                      }))}
                      value={tempBrandIds}
                ...
                <TableHead className={cn(TABLE_LAYOUT.checkbox.className, "whitespace-nowrap text-foreground font-bold uppercase text-xs bg-muted !border-r-0 p-0")}>
                  <div className="flex items-center justify-center h-full w-full px-2">
                    <Checkbox
                      ...
                      className={cn(partiallySelected && "data-[state=checked]:bg-muted data-[state=checked]:text-muted-foreground")}
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={(e) => {
                        ...
                        // Call onSelectItem with the item's default price
                          ...
                          <div className="px-4 py-1">
                            {itemIsSelected ? (
                              <Input
                                ...
                                value={price}
                                onChange={(value) => {
                                  onPriceChange?.(item.id, Math.max(0, (value as number) || 0));
                                }}
                                className="w-full h-8 text-sm"
                                placeholder={
